...
const SpackUpdate = ({
  ...
  const [isUpdatingSpack, setIsUpdatingSpack] = useState(false);
  const [showSpackModal, setShowSpackModal] = useState(isOpen);
  const [showResultsDialog, setShowResultsDialog] = useState(false);
  const [updateResults, setUpdateResults] = useState(null);
  const [selectedSpackStatus, setSelectedSpackStatus] = useState(null);
  ...
  const handleUpdateSpack = async () => {
    if (selectedSpackStatus === null) {
      toast({
        title: "Error",
        description: "Please select a Spack status",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const requestData = {
        ...
        setSpackTo: selectedSpackStatus === "yes",
      ...
      console.log("Sending spack update request with data:", requestData);
      
      const response = await fetch(`http://${import.meta.env.VITE_IP_ADDRESS}:3001/clients/update-spack`, {
        method: "POST",
        headers: {
      ...
      console.log("Spack update response status:", response.status);
      const data = await response.json();
      console.log("Spack update response data:", data);
      ...
      toast({
        title: "Success",
        description: "Spack status updated successfully",
        duration: 3000,
      });
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to update spack status",
        variant: "destructive",
      });
    } finally {
      setIsUpdatingSpack(false);
    }
  };
  ...
  const fetchPreviewCounts = async () => {
    try {
      ...
      const response = await fetch(`http://${import.meta.env.VITE_IP_ADDRESS}:3001/clients/preview-spack-update`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
  ...
  const handleOpenSpackModal = () => {
    setShowSpackModal(true);
    setSelectedSpackStatus(null);
    setPreviewCounts(null);
    setIsLoadingPreview(true);
    setClientIdsText("");
    // Set initial active tab based on whether rows are selected
    setActiveTab(hasSelectedRows ? "selected" : "filter");
    fetchPreviewCounts();
  };
  ...
  return (
    <>
      {/* Spack Status Modal */}
      <Dialog open={showSpackModal} onOpenChange={handleClose}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Update Spack Status</DialogTitle>
            <DialogDescription asChild>
              <div className="space-y-4">
                  ...
                  <TabsContent value="filter">
                    {isLoadingPreview ? (
                      ...
                      <div className="text-sm text-gray-600">
                        Set the spack status for all clients in the current filtered view.
                      </div>
                    )}
                  </TabsContent>
                ...
                <div className="py-6">
                  <RadioGroup
                    value={selectedSpackStatus}
                    onValueChange={setSelectedSpackStatus}
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={handleClose}
              disabled={isUpdatingSpack}
            >
              Cancel
            </Button>
            <Button
              onClick={handleUpdateSpack}
              disabled={
                isUpdatingSpack || 
                selectedSpackStatus === null || 
                (activeTab === "filter" && previewCounts?.totalClients === 0) ||
                (activeTab === "specific" && !validateClientIds()) ||
                (activeTab === "selected" && !hasSelectedRows)
              }
              className={isUpdatingSpack ? "opacity-50 cursor-not-allowed" : ""}
            >
              {isUpdatingSpack ? "Updating..." : `Update ${
                activeTab === "filter" 
                  ? `${previewCounts?.totalClients || ''} Records` 
                  : activeTab === "selected"
      ...
      <Dialog open={showResultsDialog} onOpenChange={setShowResultsDialog}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Spack Update Results</DialogTitle>
            <DialogDescription asChild>
              {updateResults && (
                <div className="space-y-4">
                  <dl className="text-sm grid grid-cols-2 gap-2">
