...
const Mailing = ({
  ...
  // Get rows from table - check for selection first, then fall back to all available rows
  ...
  // Set default start/end IDs when available rows change
  useEffect(() => {
    if (hasAvailableRows) {
  ...
  }, [availableRows]); // Rerun when available rows change
  ...
  const generateChecklistHTML = (columns, rowsToUse) => {
  ...
  return (
    <div className="flex justify-between">
      ...
      <div className="flex gap-2">
        ...
        >
            Print Mailing Label ({availableRows.length})
        </Button>
            </div>
          )}
      <Modal isOpen={modalOpen} onClose={closeModal}>
        ...
        <div className="flex flex-col items-center ">
          ...
          <div className="mb-4">
            ...
            >
              {/* Render actual data for Label 1 and Label 2 if available */}
              {[0, 1].map((idx) => {
                const row = availableRows[idx]?.original;
                if (!row) {
                          return (
                    <div
                      key={idx}
                      className="address-container-preview border border-gray-300 absolute text-gray-400 flex items-center justify-center"
                  style={{
                        left: `${leftPosition + idx * (columnWidth + horizontalSpacing)}px`,
                        top: `${topPosition}px`,
                    width: `${columnWidth}px`,
                ...
                // Render actual label content
                // Get wmmData for copies and subsdate
                const wmmData = row?.wmmData;
                const copies = wmmData?.totalCopies ?? "N/A";
                let subsdate = "N/A";
                if (wmmData?.subsdate) {
                  const date = new Date(wmmData.subsdate);
                  if (!isNaN(date.getTime())) {
                ...
                return (
                  <div
                    key={idx}
                    className="address-container-preview border border-gray-300 absolute bg-white"
                  style={{
                      left: `${leftPosition + idx * (columnWidth + horizontalSpacing)}px`,
                      top: `${topPosition}px`,
                      width: `${columnWidth}px`,
                  ...
                  >
                    <p>
                      {row.id || ""} - {subsdate} - {copies}cps/{row.acode || ""}
                    </p>
                    <p>{getFullName(row)}</p>
                    <p>{row.address || ""}</p>
                        {selectedFields.includes("contactnos") && (
                      <p>{getContactNumber(row)}</p>
                    )}
                  </div>
                );
              })}
