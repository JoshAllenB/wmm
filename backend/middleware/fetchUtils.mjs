...
router.post("/labels-add", verifyToken, async (req, res) => {
  try {
    const { id, description, left, width, height, columns, init, format, reset, type, printer } = req.body;
...
router.put("/labels-edit/:id", verifyToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { description, left, width, height, columns, init, format, reset, type, printer, newId } = req.body;
...
router.post("/print-dot-matrix", verifyToken, async (req, res) => {
  try {
    ...
    if (!data || !Array.isArray(data) || data.length === 0) {
      return res.status(400).json({ error: "Data array is required for printing" });
    }
    ...
    try {
      ...
      try {
        ...
        const tempFilePath = path.join(os.tmpdir(), `dotmatrix-print-${Date.now()}.prn`);
        ...
        } else if (finalPrinterConfig.type === 'usb' && finalPrinterConfig.queueName) {
        ...
        throw new Error(`Both direct printing and CUPS/lpr fallback failed: ${printerError.message}, ${cupsError.message}`);
      }
    }
  } catch (err) {
...
function processTemplateFormat(formatStr, data) {
  ...
  result = result.replace(/<<TRANSFORM\(([^,]+),\s*"([^"]+)"\)>>/g, (match, field, format) => {
  ...
  result = result.replace(/<<SYS_CDate\(([^,]+),\s*"([^"]+)"\)>>/g, (match, field, format) => {
    const value = data[field] || '';
  ...
  result = result.replace(/<<STR_Name\(([^,]+),([^,]+),([^,]+),([^,]+),([^,]+),\s*"([^"]+)"\)>>/g, 
  ...
  result = result.replace(/<<STR_MLINE\(([^,]+),(\d+),(\d+)\)>>/g, 
  ...
  // Simplified implementation
  result = result.replace(/<<STR_Check\((\d+),[^)]+\)>>/g, 
    (match, numLines) => {
...
router.get("/discover-printers", verifyToken, async (req, res) => {
  try {
    ...
    // Discover printers
    console.log(`Discovering printers (includeNetworkScan: ${includeNetworkScan})...`);
