...
const buildDateFilterStages = (startYear, endYear, modelName) => {
  ...
  const startDate = startYear
    ? new Date(Date.UTC(parseInt(startYear), 0, 1))
    : null;
  const endDate = endYear
    ? new Date(Date.UTC(parseInt(endYear), 11, 31, 23, 59, 59, 999))
    : null;
  ...
  const modelConfig = dateFields[modelName.toLowerCase()] || {
    fields: [],
    hasPaymtRef: false,
  };
  const stages = [];
...
export const getAllPayments = async (req, res) => {
  try {
    ...
    const {
      page = 1,
      limit = 20, // Changed default to match frontend
      startDate,
      endDate,
      startYear,
      endYear,
    ...
    // Get payments data (reusing the existing logic)
    const { paymentsWithClientInfo, totalRecords } = await getPaymentsData({
      startYear,
      endYear,
      search,
    });
    ...
    // Get paginated data
    const paginatedPayments = paymentsWithClientInfo.slice(
      startIndex,
      endIndex
    );
...
// GET /payments/all - Get all payments without pagination
export const getAllPaymentsUnpaginated = async (req, res) => {
  console.log("using payments/all logic");
  try {
    const startTime = Date.now();
    res.setHeader("Content-Type", "application/json");
    ...
    // Generate cache key
    const cacheKey = `unpaginated_${generateCacheKey(req.query)}`;
    ...
    // Get payments data (reusing the existing logic)
    const { paymentsWithClientInfo } = await getPaymentsData({
      startYear,
      endYear,
      search,
    });
    ...
    console.error("Error in getAllPaymentsUnpaginated:", error);
    res.status(500).json({ error: "Internal server error" });
  }
};
...
async function getPaymentsData({ startYear, endYear, search, sort, order }) {
  ...
  // Get relevant models
  const relevantModels = Object.entries(models).filter(([modelName]) => {
    const validModels = ["wmm", "hrg", "fom", "cal"];
    const isValidModel = validModels.some((valid) =>
  ...
  if (parsedSearch.paymentRef) {
    ...
    if (Object.keys(yearFilter).length > 0) {
      flatPayments = flatPayments.filter((payment) => {
        if (payment.modelType.toLowerCase().includes("wmm")) {
          return payment.extractedDates?.some(
            (date) =>
    ...
    const clientIds = [
      ...new Set(flatPayments.map((p) => p.clientid || p.clientId)),
    ];
              ...
              $addFields: {
                extractedPaymtFormDate: {
                  $function: {
                    body: function (paymtform) {
                      if (!paymtform) return null;
                      const match = String(paymtform).match(
                        /(\d{1,2})\/(\d{1,2})\/(\d{2})/
                      );
                      if (match) {
                        const [_, month, day, year] = match;
          ...
          pipeline.push({
            $project: {
              ...
              ...(isHrgModel || isFomModel
                ...
                : {}),
              ...(!isCalModel && !isWmmModel && !isHrgModel && !isFomModel
                ? { recvdate: 1 }
                : {}),
            },
          });
  ...
  const paymentsWithClientInfo = flatPayments.map((payment) => {
    ...
    if (!displayDate) {
      if (
        payment.modelType.toLowerCase().includes("wmm") &&
        payment.extractedDates?.length
      ) {
        ...
        payment.modelType.toLowerCase().includes("hrg") ||
        payment.modelType.toLowerCase().includes("fom")
      ) {
